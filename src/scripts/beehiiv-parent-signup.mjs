...
export function initBeehiivParentSignup(options = {}) {
  ...
  if (win.top === win && isBeehiivSignupResult(win.location.href, win.location.origin, win.location.href)) {
    ...
    if (!pending || !pending.meta || !Number.isFinite(age) || age < 0 || age > PENDING_TTL_MS) {
      ...
      // A validated, recent form-return may show receipt guidance even when
      // analytics is unavailable. This does not assert email confirmation.
      doc.querySelectorAll('.capture__success').forEach((panel) => {
        panel.hidden = false;
        const capture = panel.closest('.capture-cta') || panel.parentNode;
        capture?.querySelectorAll('.capture__embed-window,.capture__embed,.capture__fallback,.capture__helper')
          .forEach((element) => { element.hidden = true; });
      });
      if (fireSignup(pending.meta)) {
        try { storage?.removeItem(PENDING_KEY); } catch {}
      }
    }
  }
