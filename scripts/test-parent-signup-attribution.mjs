...
function harness({ frames = [], panels = [], href = 'https://cassiancreed.com/post/example/', withGtag = true, storage = new MemoryStorage() } = {}) {
  const events = [];
  const location = new URL(href);
  const listeners = new Map();
  const document = {
    selector: null,
    querySelectorAll(selector) { this.selector = selector; return selector === '.capture__success' ? panels : frames; },
...
test('GA4 loader and page_view are gated to canonical top-level pages', async () => {
  ...
  assert.match(base, /origin !== 'https:\/\/cassiancreed\.com' && origin !== 'https:\/\/www\.cassiancreed\.com'/);
  ...
  const execute = ({ origin, isTopLevel }) => {
    const appended = [];
    const window = {};
    ...
    runInNewContext(inline, { Date, document, location: { origin }, window });
    return { appended, dataLayer: window.dataLayer || [] };
  };

  const canonical = execute({ origin: 'https://cassiancreed.com', isTopLevel: true });
  ...
  const iframe = execute({ origin: 'https://cassiancreed.com', isTopLevel: false });
  assert.equal(iframe.appended.length, 0);
  assert.equal(iframe.dataLayer.length, 0);

  const preview = execute({ origin: 'https://deploy-preview-149--sunny-tulumba-9894fe.netlify.app', isTopLevel: true });
  assert.equal(preview.appended.length, 0);
  assert.equal(preview.dataLayer.length, 0);

  const insecure = execute({ origin: 'http://cassiancreed.com', isTopLevel: true });
  assert.equal(insecure.appended.length, 0);
  assert.equal(insecure.dataLayer.length, 0);

  const nonstandardPort = execute({ origin: 'https://cassiancreed.com:444', isTopLevel: true });
  assert.equal(nonstandardPort.appended.length, 0);
  assert.equal(nonstandardPort.dataLayer.length, 0);
});


test('fallback receipt UI requires valid intent and does not assert confirmed delivery', () => {
  function ui() {
    const controls = [{ hidden: false }, { hidden: false }];
    const capture = { querySelectorAll: () => controls };
    const panel = { hidden: true, closest: () => capture };
    return { controls, panel };
  }
  const direct = ui();
  harness({ href: 'https://cassiancreed.com/?subscribed=1', panels: [direct.panel] });
  assert.equal(direct.panel.hidden, true);
  assert.ok(direct.controls.every((control) => !control.hidden));
  const storage = new MemoryStorage();
  storage.setItem('nep_signup_pending_v1', JSON.stringify({ created_at: 999_999, meta: { form_id: 'guide' } }));
  const invalid = ui();
  harness({ href: 'https://cassiancreed.com/?not_subscribed=1', storage, panels: [invalid.panel] });
  assert.equal(invalid.panel.hidden, true);
  const valid = ui();
  const result = harness({ href: 'https://cassiancreed.com/?subscribed=1', storage, panels: [valid.panel] });
  assert.equal(valid.panel.hidden, false);
  assert.ok(valid.controls.every((control) => control.hidden));
  assert.equal(result.events.length, 1);
  const reload = ui();
  const repeated = harness({ href: 'https://cassiancreed.com/?subscribed=1', storage, panels: [reload.panel] });
  assert.equal(reload.panel.hidden, true);
  assert.equal(repeated.events.length, 0);
  const blockedStorage = new MemoryStorage();
  blockedStorage.setItem('nep_signup_pending_v1', JSON.stringify({ created_at: 999_999, meta: { form_id: 'guide' } }));
  const blocked = ui();
  harness({ href: 'https://cassiancreed.com/?subscribed=1', storage: blockedStorage, panels: [blocked.panel], withGtag: false });
  assert.equal(blocked.panel.hidden, false);
});
