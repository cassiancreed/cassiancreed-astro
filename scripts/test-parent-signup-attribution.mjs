...
test('acquisition landing path rejects protocol-relative roots', async () => {
  const base = await readFile(new URL('../src/layouts/Base.astro', import.meta.url), 'utf8');
  const inline = base.match(/ORIGIN CAPTURE[\s\S]*?<script is:inline>\s*([\s\S]*?)\s*<\/script>/)?.[1];
  assert.ok(inline, 'origin-capture inline script not found');
  ...
  runInNewContext(inline, {
    ...
    gtag: (...args) => events.push(args),
    location: {
      search: '?utm_source=security-test&utm_campaign=user%40example.com&utm_content=%2Fpoisoned-path&utm_term=15551234567',
      pathname: '//attacker.example/poisoned',
    },
    Math,
    sessionStorage,
    window,
  ...
  const stored = JSON.parse(values.get('nep_origin'));
  assert.equal(stored.p, '/');
  assert.equal(window.NEP_ORIGIN.p, '/');
  assert.equal(events[0][1], 'origin_visit');
  assert.equal(events[0][2].origin_source, 'security-test');
  assert.equal(events[0][2].origin_campaign, '(none)');
  assert.equal(events[0][2].origin_content, '(none)');
  assert.equal(events[0][2].origin_term, '(none)');
