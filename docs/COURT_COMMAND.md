...
The version 2 JSON contract is defined by
`src/data/NEP-Court-Command-Intake-Bundle-Schema-v2.json`; a validator-ready
example lives at `src/data/court-command-intake.example.json`. It requires a stable
case key and aliases; separated legal truth, demand, commercial opportunity, and
public display sections; court and predictive events; timestamped court, media,
creator, search, social, and NEP signals; SEO, observed PAA versus candidates, AEO
and AI-search prompts; buyer questions; canonical URLs; field freshness; and
destination version/cutoff state. URLs remain evidence leads until verified.

After promotion, `case-intelligence.json` is the canonical operating record. A
single packet joins domestic and international calendar rows, derives preparation,
pre-event, monitoring, immediate-response, 12–48-hour follow-up, and
search/streaming-tail windows, and calculates destination-specific field hashes.
The distribution ledger records destination, version, factual cutoff, and the
hashes used. A later change regenerates only destinations whose dependencies
changed. Demand and commercial fields are internal decision support and must not
